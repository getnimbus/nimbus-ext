<script lang="ts">
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        triggerToast(
          "Successfully add tag to your custom category!",
          "success"
        );
      }
    } catch (e) {
  ...
  const deleteCustomCategory = async () => {
    ...
    try {
      ...
      triggerToast("Successfully delete your custom category!", "success");
    } catch (e) {
      console.error("e:", e);
      triggerToast(
  ...
  $: {
    if (!$queryTokenHolding.isFetching && dataSubWS && dataSubWS.length !== 0) {
      let filteredData = [];
