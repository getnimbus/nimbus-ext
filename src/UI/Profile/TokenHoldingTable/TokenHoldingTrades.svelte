<script lang="ts">
  ...
  $: formatData = holdingTokenData
    ...
    .filter((item) => Number(item?.amount) * Number(item?.price?.price) > 0);
  ...
  const handleCalculateValue = (data: any) => {
    return Number(data?.amount) * Number(data?.price?.price);
  };

  const handleCalculatePercentPnl = (data: any) => {
...
>
  ...
  >
    ...
    {:else}
      ...
      >
        <div
          class={`grid grid-cols-7 group transition-all ${index > 0 ? "cursor-pointer" : ""}`}
          slot="item"
          let:index
          let:style
          {style}
        ...
        >
          ...
          {:else}
            ...
            >
              ...
              >
                $<TooltipNumber
                  number={formatData[index - 1]?.price?.price}
                  type="balance"
                />
              </div>
            </div>
            ...
            >
              <div class="flex flex-col gap-1">
                ...
                >
                  {#if dayjs().diff(dayjs(formatData[index - 1]?.last_transferred_at), "hour") < 24 && dayjs().diff(dayjs(formatData[index - 1]?.last_transferred_at), "hour") > 0}
                    <span
                      use:tooltip={{
                        content: `<tooltip-detail text="Changed ${dayjs().diff(
                          dayjs(formatData[index - 1]?.last_transferred_at),
                          "hour"
                        )} hrs ago" />`,
                        allowHTML: true,
                        placement: "top",
                  ...
                  {/if}
                  <TooltipNumber
                    number={formatData[index - 1].amount}
                    type="balance"
                    personalValue
                  />
                </div>
                ...
                >
                  <TooltipNumber
                    number={handleCalculateValue(formatData[index - 1])}
                    type="value"
                    personalValue
                  />
                </div>
              ...
              >
                ...
                {:else}
                  <div class="flex flex-col">
                    ...
                    >
                      <TooltipNumber
                        number={Math.abs(formatData[index - 1].pnl)}
                        type="value"
                        personalValue
                      />
                    </div>
                    <div class="flex items-center justify-end gap-1">
                      ...
                      >
                        <TooltipNumber
                          number={Math.abs(
                            handleCalculatePercentPnl(formatData[index - 1])
                          ) * 100}
                          type="percent"
                        />
                        <span>%</span>
                      </div>
                      {#if formatData[index - 1].pnl !== 0}
                        <img
                          src={formatData[index - 1].pnl >= 0
                            ? TrendUp
                            : TrendDown}
                          alt="trend"
                          class="mb-1"
  ...
  >
    ...
    {:else}
      <VirtualList
        scrollDirection="vertical"
        width="100%"
        height={formatData.length < 10 ? formatData.length * 300 : 405}
      ...
      >
        ...
        >
          ...
          <div class="flex justify-between items-start">
            <div class="text-right text-sm uppercase font-medium">Price</div>
            <div
          ...
          <div class="flex justify-between items-start">
            <div class="text-right text-sm uppercase font-medium">Amount</div>
            <div class="flex flex-col gap-1">
              <div
                class="flex items-center justify-end gap-1 font-medium text-sm text_00000099"
              >
                {#if dayjs().diff(dayjs(formatData[index]?.last_transferred_at), "hour") < 24 && dayjs().diff(dayjs(formatData[index]?.last_transferred_at), "hour") > 0}
                  <span
                    use:tooltip={{
                      content: `<tooltip-detail text="Changed ${dayjs().diff(
                        dayjs(formatData[index]?.last_transferred_at),
                        "hour"
                      )} hrs ago" />`,
                      allowHTML: true,
                      placement: "top",
                ...
                {/if}
                <TooltipNumber
                  number={formatData[index].amount}
                  type="balance"
                  personalValue
                />
              </div>
              ...
              <div class="flex justify-end font-medium text-sm text_00000099">
                <TooltipNumber
                  number={handleCalculateValue(formatData[index])}
                  type="value"
                  personalValue
                />
              </div>
            ...
            >
              ...
              {:else}
                <div class="flex flex-col">
                  ...
                  >
                    <TooltipNumber
                      number={Math.abs(formatData[index].pnl)}
                      type="value"
                      personalValue
                    />
                  </div>
                  <div class="flex items-center justify-end gap-1">
                    ...
                    >
                      <TooltipNumber
                        number={Math.abs(
                          handleCalculatePercentPnl(formatData[index])
                        ) * 100}
                        type="percent"
                      />
                      <span>%</span>
                    </div>
                    {#if formatData[index].pnl !== 0}
                      <img
                        src={formatData[index].pnl >= 0 ? TrendUp : TrendDown}
                        alt="trend"
                        class="mb-1"
                      />
                    {/if}
