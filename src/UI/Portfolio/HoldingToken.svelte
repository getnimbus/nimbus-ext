<script lang="ts">
  ...
  const handleReportTrashCoin = async () => {
    ...
    try {
      ...
      isLoadingReportTrashCoin = false;
      triggerToast(
        "Something wrong when report token. Please try again!",
        "fail"
      );
  ...
  const handleSwapToken = (data, address) => {
    const config = {
      ...
      onSuccess: ({ txid, result }) => {
        ...
        triggerToast(
          `Swap token successful. Tx id is ${shorterAddress(txid)}`,
          "success"
        );
      },
      onSwapError: ({ error }) => {
        console.log("onSwapError", error);
        triggerToast("Swap token fail. Please try again!", "fail");
      },
    };
...
>
  ...
  {:else}
    <VirtualList
      ...
      width="100%"
      height={data.length < 30 ? data.length * 75 + 1 + 40 : 2250}
      ...
      <div
        ...
        on:mouseover={() => {
          if (index > 0) {
            ...
            if (data[index - 1]?.cmc_slug) {
              selectedItemIndex = index - 1;
              isShowCMC = true;
            }
            if (data[index - 1]?.cg_id) {
              selectedItemIndex = index - 1;
        ...
        on:click={() => {
          if (index > 0) {
            if (listSupported.includes($typeWallet)) {
              ...
              selectedTokenDetail = data[index - 1];
            }
          }
        }}
      >
        {#if index === 0}
          ...
          {#if listSupported.includes($typeWallet) && $typeWallet === "BUNDLE"}
            <div class="bg_f4f5f8 py-3 rounded-tr-[10px]" />
          {/if}
        {:else}
          <div
            ...
            <div class="relative flex items-center justify-start gap-3">
              <div class="relative">
                <div class="rounded-full w-[30px] h-[30px] overflow-hidden">
                  <Image
                    logo={data[index - 1].logo}
                    defaultLogo={defaultToken}
                  />
                </div>
                {#if ($typeWallet === "EVM" || $typeWallet === "MOVE" || $typeWallet === "BUNDLE") && data[index - 1]?.chain !== "CEX"}
                  <div class="absolute -top-2 -right-1">
                    <img
                      src={detectedChain(data[index - 1]?.chain)?.logo}
                      alt=""
                      width="15"
                      height="15"
                      class="rounded-full"
              ...
              <div class="flex flex-col gap-1">
                <div class="flex items-start gap-2">
                  ...
                  >
                    {#if !data[index - 1]?.name || data[index - 1]?.name === "N/A"}
                      <span>
                        {shorterAddress(data[index - 1]?.contractAddress)}
                      </span>
                    {:else}
                      <span>
                    ...
                    {#if isShowTooltipName && selectedItemIndex === index - 1 && data[index - 1]?.name?.length > 12}
                      <div
                        class="absolute left-0 -top-8"
                        style="z-index: 2147483648;"
                      >
                        <Tooltip text={data[index - 1].name} />
                      </div>
                    {/if}
                  </div>

                  {#if handleCheckVault(data[index - 1]) !== undefined}
                    <div
                      ...
                      on:click={(e) => {
                        if (index > 0) {
                          ...
                          selectedVaults = data[index - 1].vaults;
                          selectedVaultsSymbol = data[index - 1].symbol;
                        }
                      }}
                    >
                      <div class="flex gap-1">
                        Farm up to
                        {numeral(
                          handleCheckVault(data[index - 1])?.apy * 100
                        ).format("0,0.0")}% APY
                      </div>
                    </div>
                  {/if}
                ...
                <div class="flex items-center gap-2 min-h-[20px]">
                  ...
                  >
                    {#if data[index - 1].symbol === undefined}
                      N/A
                    {:else}
                      {shorterName(data[index - 1].symbol, 12)}
                    {/if}
                    {#if isShowTooltipSymbol && selectedItemIndex === index - 1 && data[index - 1].symbol.length > 12}
                      <div
                        class="absolute left-0 -top-8"
                        style="z-index: 2147483648;"
                      >
                        <Tooltip text={data[index - 1].symbol} />
                      </div>
                    {/if}
                  </div>
                  ...
                  {#if isShowCMC && selectedItemIndex === index - 1}
                    <a
                      href={`https://coinmarketcap.com/currencies/${data[index - 1]?.cmc_slug}`}
                      on:click={(e) => {
                        if (index > 0) {
                          e.stopPropagation();
                        }
                  ...
                  {#if isShowCoingecko && selectedItemIndex === index - 1}
                    <a
                      href={`https://www.coingecko.com/en/coins/${data[index - 1]?.cg_id}`}
                      on:click={(e) => {
                        if (index > 0) {
                          e.stopPropagation();
                        }
                  ...
                  {#if data[index - 1]?.positionType === "ERC_404"}
                    <span
          ...
          >
            ...
            >
              {#if handleCheckWithinLast24Hours(data[index - 1]) < 24 && handleCheckWithinLast24Hours(data[index - 1]) > 0}
                <span
                  use:tooltip={{
                    content: `<tooltip-detail text="Changed ${handleCheckWithinLast24Hours(data[index - 1])} hrs ago" />`,
                    allowHTML: true,
                    placement: "top",
                    interactive: true,
                  }}
          ...
          {#if listSupported.includes($typeWallet) && $typeWallet === "BUNDLE"}
            <div
              ...
              style={`${data.length - 1 === index - 1 ? "border-bottom-right-radius: 10px;" : ""}`}
            >
              <div
              ...
              >
                ...
                >
                  <div
                    ...
                    class:rotate-0={listSelectedIndex.includes(index - 1)}
                    on:click={(e) => {
                      if (index > 0) {
                        e.stopPropagation();
                        listSelectedIndex.includes(index - 1)
          ...
          {#if listSelectedIndex.includes(index - 1)}
            ...
            >
              {#each handleFormatDataBreakdown(data[index - 1]) as item}
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <TooltipNumber
                            number={Math.abs(
                              item.amount / data[index - 1].amount
                            ) * 100}
                            type={Math.abs(
                              item.amount / data[index - 1].amount
                            ) *
...
>
  <div class="h-[563px] flex flex-col gap-4">
    <div class="font-medium text-2xl">Token breakdown</div>
    <div
    ...
    >
      <table
        class={`table-auto xl:w-full w-[700px] ${
        ...
        <thead>
          <tr class="bg_f4f5f8 sticky left-0 top-0">
            <th class="pl-3 py-3 rounded-tl-[10px]">
              <div class="font-medium text-left uppercase text-sm">Account</div>
            </th>
            <th class="py-3">
              <div class="font-medium text-right uppercase text-sm">Amount</div>
            </th>
            <th class="py-3 pr-3 rounded-tr-[10px]">
              <div class="font-medium text-right uppercase text-sm">Value</div>
            </th>
          </tr>
        </thead>
        <tbody>
          ...
          {:else}
            {#each handleFormatDataBreakdown(selectedTokenDetail) as item}
              <tr class="transition-all cursor-pointer group">
                ...
                >
                  <div
                    class="text-sm text_00000099 flex flex-col justify-end items-end gap-2"
                  >
                    <div
                      class="flex justify-end items-center gap-1 font-medium text-sm text_00000099"
                    >
                      <TooltipNumber number={item?.amount} type="balance" />
                ...
                >
                  <div
                    class="text-sm text_00000099 font-medium flex justify-end items-center"
                  >
                    $<TooltipNumber
                      number={Number(item?.amount) *
...
>
  <div class="flex flex-col gap-4 xl:mt-0 mt-4">
    <div class="font-medium title-3">Blacklist Token</div>
    <form
        ...
        >
          ...
          <div class="flex items-center gap-2 cursor-pointer w-max">
            <input
              ...
              class="cursor-pointer relative xl:w-4 xl:h-4 w-6 h-6 appearance-none rounded-[0.25rem] border outline-none before:pointer-events-none before:absolute before:h-[0.875rem] before:w-[0.875rem] before:scale-0 before:rounded-full before:bg-transparent before:opacity-0 before:shadow-[0px_0px_0px_13px_transparent] before:content-[''] checked:border-primary checked:bg-primary checked:before:opacity-[0.16] checked:after:absolute checked:after:-mt-px checked:after:ml-[0.25rem] checked:after:block checked:after:h-[0.8125rem] checked:after:w-[0.375rem] checked:after:rotate-45 checked:after:border-[0.125rem] checked:after:border-l-0 checked:after:border-t-0 checked:after:border-solid checked:after:border-white checked:after:bg-transparent checked:after:content-[''] hover:cursor-pointer hover:before:opacity-[0.04] hover:before:shadow-[0px_0px_0px_13px_rgba(0,0,0,0.6)] focus:shadow-none focus:transition-[border-color_0.2s] focus:before:scale-100 focus:before:opacity-[0.12] focus:before:shadow-[0px_0px_0px_13px_rgba(0,0,0,0.6)] focus:before:transition-[box-shadow_0.2s,transform_0.2s] focus:after:absolute focus:after:z-[1] focus:after:block focus:after:h-[0.875rem] focus:after:w-[0.875rem] focus:after:rounded-[0.125rem] focus:after:content-[''] checked:focus:before:scale-100 checked:focus:before:shadow-[0px_0px_0px_13px_#3b71ca] checked:focus:before:transition-[box-shadow_0.2s,transform_0.2s] checked:focus:after:-mt-px checked:focus:after:ml-[0.25rem] checked:focus:after:h-[0.8125rem] checked:focus:after:w-[0.375rem] checked:focus:after:rotate-45 checked:focus:after:rounded-none checked:focus:after:border-[0.125rem] checked:focus:after:border-l-0 checked:focus:after:border-t-0 checked:focus:after:border-solid checked:focus:after:border-white checked:focus:after:bg-transparent dark:border-neutral-600 dark:checked:border-primary dark:checked:bg-primary dark:focus:before:shadow-[0px_0px_0px_13px_rgba(255,255,255,0.4)] dark:checked:focus:before:shadow-[0px_0px_0px_13px_#3b71ca]"
              on:change={(event) => {
                isOldToken = event.target.checked;
              }}
            />
            <label
              for="outdated"
